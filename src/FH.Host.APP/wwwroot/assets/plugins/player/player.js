...
(function (a) {
  ...
  var f = (c.event.special.mousewheel = {
    ...
    getLineHeight: function (i) {
      return parseInt(
        c(i)
          ["offsetParent" in c.fn ? "offsetParent" : "parent"]()
          .css("fontSize"),
        10
      );
    },
  ...
  function a(m, i) {
    return (
      f.settings.adjustOldDeltas && m.type === "mousewheel" && i % 120 === 0
    );
  }
});
/*scrollbar plugin*/
      ...
      scrolling: function (i, q, n, k, A, f, D, w) {
        ...
        if (!l.data("bindEvent_scrollbar_drag")) {
          ...
          if (c.support.pointer || c.support.msPointer) {
            ...
            c(document)
              .bind("mousemove." + l.data("mCustomScrollbarIndex"), function (
                K
              ) {
                if (l.data("on_drag")) {
                  var J = k,
...
var lzxPlayerInit = function () {
  ...
  var testKey =
    typeof lzxPlayerTestKey !== "undefined" ? lzxPlayerTestKey : null;
  var styleLoaded =
    typeof lzxPlayerStyleLoaded !== "undefined" && lzxPlayerStyleLoaded;
  if (
    navigator.userAgent.match(
      /(iPhone|iPod|Android|ios|Nokia|Black Berry|MIDP|Phone)/i
    )
  ) {
    isPhone = true;
  }
  ...
  if (!isTest) {
    ...
    isLoad =
      isLoad &&
      typeof lastFeed !== "undefined" &&
      new Date().getTime() - parseInt(lastFeed) < 2000;
  ...
  var webURL = jsUrl.startsWith("http")
    ? jsUrl.substring(0, jsUrl.indexOf("/", 8))
  ...
  if (!styleLoaded) {
    ...
    $("head").append(
      '<link href="https://libs.baidu.com/fontawesome/4.2.0/css/font-awesome.css" rel="stylesheet" type="text/css">'
    );
  ...
  (songFrom33 = "开启"),
    ...
    (ycgeci = true),
    (first = 1),
    (volume = $.cookie("lzx_player_volume")
  ...
  var lzxMedia = {
    play: function () {
      ...
      cicleTime = setInterval(function () {
        if (!rateMouse.isDown && !rateTouch.isTouchDown) {
          $songTime.text(
            formatSecond(audio.currentTime) +
              " / " +
              formatSecond(audio.duration)
          );
          $(".rate-on", $rateSlider).width(
        ...
        if (audio.currentTime < audio.duration / 2) {
          $btns.css(
            ...
            "linear-gradient(90deg, " +
              ...
              (90 + ((270 - 90) / (audio.duration / 2)) * audio.currentTime) +
              "deg, " +
              lightcolor +
              " 50%, " +
              roundcolor +
        ...
        if (timeRanges.length !== 0) {
          $rateBuffered.width(
            parseInt(
              ((timeRanges.end(timeRanges.length - 1) * 100) / audio.duration) *
                100
            ) /
              100 +
              "%"
    ...
    error: function () {
      ...
      $player.removeClass("playing");
      lzxTips.show(
        songSheetList[albumId].songNames[songId] +
          " - 资源获取失败！尝试获取下一首..."
      );
      lzxMedia.next();
    },
    ...
    next: function () {
      clearInterval(cicleTime);
      random
        ? lzxMedia.getInfos(window.parseInt(Math.random() * songTotal))
  ...
  $volumeSlider.click(function (e) {
    ...
    var calcVolume = (
      1 -
      (e.clientY - progressOffsetTop) / progressHeight
    ).toFixed(2);
  ...
  $rateSlider.click(function (e) {
    ...
    audio.currentTime =
      audio.duration * ((eClientX - progressOffsetLeft) / progressWidth);
  });
  var isDown = false;
  ...
  $(".drag", $rateSlider).on("touchstart", function (e) {
    ...
    rateTouch.rateOnWidth = parseFloat(
      ($(".rate-on", $rateSlider).width() / rateTouch.progressWidth).toFixed(2)
    );
  });
  ...
  $(".drag", $rateSlider).mousedown(function (e) {
    ...
    rateMouse.rateOnWidth = parseFloat(
      ($(".rate-on", $rateSlider).width() / rateMouse.progressWidth).toFixed(2)
    );
  });
    ...
    function (e) {
      if (rateTouch.isTouchDown) {
        var rate =
          parseFloat(
            (
              (e.touches[0].clientX - rateTouch.startX) /
              rateTouch.progressWidth
            ).toFixed(2)
          ) + rateTouch.rateOnWidth;
        if (rate >= 0 && rate <= 1) {
          $(".rate-on", $rateSlider).width(rate * 100 + "%");
          rateTouch.currentTime = audio.duration * rate;
          $songTime.text(
            formatSecond(rateTouch.currentTime) +
              " / " +
              formatSecond(audio.duration)
          );
        }
    ...
    mousemove: function (e) {
      if (isDown) {
        ...
        var progressHeight = $volumeSlider.height(),
          ...
          eClientY = e.clientY;
        if (
          eClientY >= progressOffsetTop &&
          eClientY <= progressOffsetTop + progressHeight
        ) {
          audio.volume = (
            1 -
            (eClientY - progressOffsetTop) / progressHeight
        ...
        var rate =
          parseFloat(
            ((e.clientX - rateMouse.startX) / rateMouse.progressWidth).toFixed(
              2
            )
          ) + rateMouse.rateOnWidth;
        if (rate >= 0 && rate <= 1) {
          $(".rate-on", $rateSlider).width(rate * 100 + "%");
          rateMouse.currentTime = audio.duration * rate;
          $songTime.text(
            formatSecond(rateMouse.currentTime) +
              " / " +
              formatSecond(audio.duration)
          );
        }
  ...
  $(".song-list .musicheader,.song-list .fa-angle-right", $player).click(
    function () {
    ...
    creat: {
      album: function () {
        ...
        for (var c = 0; c < albumTotal; c++) {
          albumList +=
            '<li><i class="fa fa-angle-right"></i><span class="index">' +
            (c + 1) +
            '</span><span class="artist"></span>《' +
            songSheetList[c].songSheetName +
            "》 - " +
            songSheetList[c].author +
            "</li>";
        }
        $(".list", $albumList)
        ...
        $("li", $albumList).click(function () {
          var a = $(this).index();
          $(this).hasClass(cur)
            ? lzxPlayer.playList.creat.song(a, true)
      ...
      song: function (id, isThisAlbum) {
        ...
        $(".song-list .musicheader span", $player).text(
          songSheetList[id].songSheetName + "(" + songTotal + ")"
        );
        var songList = "";
        ...
        if (isThisAlbum) {
          ...
          $songList.mCustomScrollbar(
            "scrollTo",
            $("li.current", $songList).position().top - 120
          );
        } else {
        ...
        $("li", $songList).click(function () {
          ...
          if ($(this).hasClass(cur)) {
            lzxTips.show(
              "正在播放 - " +
                songSheetList[albumId].songNames[songId].replace(
                  songId + 1 + "#",
                  ""
                )
            );
  ...
  var lzxLrc = {
    load: function () {
      ...
      setTimeout(function () {
        if (hasgeci) {
          $songFrom3.html(
            '<i class="fa fa-check-circle"></i> 歌词' + songFrom33
          );
        } else {
          $songFrom3.html(
            '<i class="fa fa-times-circle"></i> 歌词' + songFrom33
          );
        }
        $(".switch-down").css("right", "65px");
        $(".switch-ksclrc").show();
        $.ajax({
          url:
            webURL +
            "/api/musicLyric?songId=" +
            songSheetList[albumId].songIds[songId] +
            "&type=" +
      ...
      format: function (cont) {
        ...
        for (var i = 1; i < lrcCont.length; i += 2) {
          ...
          if (i == 1) {
            ...
            lrcLine +=
              '<li class="lzxLrc' + timer + '">' + lrcCont[i + 1] + "</li>";
          }
        }
  ...
  dogInterval = setInterval(function () {
    ...
    // 检查css变量
    var currPlayerWidth = document.body.style.getPropertyValue(
      "--player-width"
    );
    if (
      typeof playerWidth != "undefined" &&
      playerWidth !== -1 &&
      currPlayerWidth != playerWidth + "px"
    ) {
      document.body.style.setProperty("--player-width", playerWidth + "px");
    }
    var currCoverWidth = document.body.style.getPropertyValue("--cover-width");
    if (
      typeof coverWidth != "undefined" &&
      coverWidth !== -1 &&
      currCoverWidth != coverWidth + "px"
    ) {
      document.body.style.setProperty("--cover-width", coverWidth + "px");
    }
  }, 1000);
  ...
  function netmusic() {
    ...
    $(".switch-down").show();
    $(".switch-down").html(
      '<a class="down"><i class="fa fa-cloud-download" title="从' +
        songFrom55 +
        "下载：" +
        songSheetList[albumId].songNames[songId] +
        " - " +
    ...
    $songName.html(
      '<span title="' +
        songSheetList[albumId].songNames[songId] +
        '">' +
        LimitStr(songSheetList[albumId].songNames[songId]) +
        "</span>"
    );
    window.console.log(
      name +
    ...
    $songFrom.html(
      '<span title="' +
        songSheetList[albumId].artistNames[songId] +
        '">' +
        LimitStr(songSheetList[albumId].artistNames[songId]) +
        "</span>"
    );
    $songFrom1.html(
      '<span title="' +
        songSheetList[albumId].albumNames[songId] +
        '">' +
    ...
    coverImg.error = function () {
      ...
      setTimeout(function () {
        lzxTips.show(
          songSheetList[albumId].songNames[songId] + " - 专辑图片获取失败！"
        );
      }, 4000);
    };
    $cover.html(coverImg);
    if (background == 1) {
      $(".blur-img .blur", $player).attr(
        "src",
        songSheetList[albumId].albumCovers[songId]
      ); //虚化背景
    } else {
    ...
    if (first == 1) {
      ...
      if (
        autoPlayer == 1 &&
        ($.cookie("auto_playre") == null || $.cookie("auto_playre") === "yes")
      ) {
        startPlay();
      } else {
